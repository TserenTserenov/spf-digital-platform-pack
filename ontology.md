...
| **Разборщик** (= Note-Review) | Ежедневный процесс классификации fleeting-заметок по категориям (НЭП, задача, знание, идея, информация, шум) с маршрутизацией предложений в целевые документы | U.Method | DP.AGENT.012, DP.AISYS.013 |
| **Архитектурная характеристика** | Измеряемое качество архитектуры (эволюционируемость, масштабируемость, обучаемость, генеративность) | U.Characteristic | DP.D.010 |
...
| Архитектура (ARCH) | содержит → | ИИ-система (AGENT) | DP.ARCH.001 (Слой 2, зона А) → DP.AGENT.001 |
...
├── Слой 2: Обработка
│   ├── Зона А: ИИ-системы (AGENT/AISYS) — LLM, stateless
