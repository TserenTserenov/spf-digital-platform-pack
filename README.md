...
## Основные идеи (v0.2.0)

### 4-слойная архитектура

```
Слой 4: Интерфейсы                — тонкие клиенты (Telegram, LMS, ChatGPT, API)
Слой 3: ИИ-системы                — агенты + ассистенты (LLM-зависимые, stateless)
Слой 2: Детерминированные системы — CRM, LMS, Биллинг, ORY, MCP-серверы
Слой 1: Хранилище                 — БД, файлы, индексы
```

**ИИ-системы** и **детерминированные системы** — виды **систем платформы** (D.DTP.015). Каждый слой взаимодействует только с соседним. Интерфейс не лезет в хранилище, ИИ-система не знает какой UI у пользователя.
...
**ИИ-агент** и **ИИ-ассистент** — это **виды** ИИ-систем (D.DTP.009), а не разные слои. Вид определяется тремя признаками: ориентация (человек/система), инициатива (по запросу/автономно), интерфейс (диалог/API).
...
└── pack/
    └── digital-platform/
        ...
        │   └── 01B-distinctions.md    #   14 различений (D.DTP.001–014)
        ├── 02-domain-entities/        # Сущности домена
        │   ├── DP.ARCH.001            #   Архитектура (4 слоя, 3 характеристики)
...
|---|-----------|-------------|
| D.DTP.009 | ИИ-агент ≠ ИИ-ассистент | Виды ИИ-систем, не разные слои |
| D.DTP.010 | Характеристика ≠ Принцип | Цель (измеряемое) vs средство (правило) |
| D.DTP.011 | Platform-space ≠ User-space | Общее vs персональное |
| D.DTP.012 | Знание ≠ Информация | Формализовано vs сырое |
| D.DTP.015 | ИИ-система ≠ ИТ-система | LLM-зависимая vs детерминированная |
...
*Pack v0.2.0 | Построен по [SPF](https://github.com/TserenTserenov/SPF) (Second Principles Framework)*
